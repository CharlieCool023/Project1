...
export async function addProductToBlockchain(
  ...
  try {
    ...
    const response = await axiosInstance.post(`/${KALEIDO_CONTRACT_ADDRESS}/addProduct`, {
      batchNumber,
      productName,
      manufacturingDate,
      expiryDate,
    ...
    if (axios.isAxiosError(error)) {
      if (error.response) {
        ...
        throw new Error(`Failed to add product to blockchain: ${error.response.status} ${JSON.stringify(error.response.data)}`);
      ...
      throw new Error(`Failed to add product to blockchain: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
}
...
export async function getProductFromBlockchain(productId: string): Promise<Product | null> {
  try {
    const response = await axiosInstance.post(`/${KALEIDO_CONTRACT_ADDRESS}/getProduct`, {
    ...
    if (response.data && response.data.output) {
      // The output should be an object with the product details
      const product = response.data.output;
      if (typeof product === 'object' && product.batchNumber) {
    ...
    if (axios.isAxiosError(error) && error.response) {
      throw new Error(`Failed to get product from blockchain: ${error.response.status} ${JSON.stringify(error.response.data)}`);
