  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      if (error.response) {
        ...
        throw new Error(
          `Failed to upload file to IPFS: ${error.response.status} ${JSON.stringify(error.response.data)}`,
        )
...
export function getIPFSImageUrl(hash: string): string {
  ...
  return `${KALEIDO_IPFS_URL}/ipfs/${hash}`
}

export async function fetchIPFSImage(hash: string): Promise<string> {
  if (!hash) return "/placeholder.svg"
  try {
    const response = await axiosInstance.get(`/ipfs/${hash}`, {
